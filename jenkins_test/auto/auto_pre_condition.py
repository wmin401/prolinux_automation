...
if __name__ == '__main__':
    ...
    onlyPreCondition = True

    if onlyPreCondition == False:
        print("Pre condition만 테스트하고 싶을 경우 onlyPreCondition True로 변경")
    else:
        daemonWithPreCondition = PRE_CONDITIONS()
        if VERSION_DETAIL == 'v8_2' or VERSION_DETAIL == 'v7_8':
            for i in range(1,9):
                #daemonWithPreCondition.Apply('list/'+VERSION_DETAIL+'/release/daemon/test_di.txt', i)
                daemonWithPreCondition.Restore('list/'+VERSION_DETAIL+'/release/daemon/pre_condition/daemon_for_pre_condition_%d.txt'%(i))
        else:
            #daemonWithPreCondition.Apply('list/'+VERSION_DETAIL+'/release/daemon/test_di.txt', 0)
            daemonWithPreCondition.Restore('list/'+VERSION_DETAIL+'/release/daemon/test_di.txt')

    h, m, s = secToHms(start, time.time())
    print("Pre condition Running Time : %dh %dm %.2fs"%(h, m, s))
